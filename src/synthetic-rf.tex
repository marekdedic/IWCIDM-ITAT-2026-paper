\section{RF-based Methods for Graph HPO}

In this section, we introduce the metalearning Synthetic-RF method, a novel approach for HPO specifically designed for graph learning, building on our previous metalearning works \cite{dedic_benchmarking_2026, prochazka_which_2023}. This section first introduces the Cross-RF (\name{cross-dataset random forest}) method from those previous works, followed by the newly-proposed Synthetic-RF method.
...
\begin{equation*}
	\begin{split}
		\mathcal{A} = \biggl\{ \left( \delta \left( \mathcal{D}^{(1)} \right), \lambda^{(1)}, \rho( \mathvec{y}_{\mathcal{D}^{(1)}}, \hat{f}_{\lambda^{(1)}} \left( \mathmat{X}_{\mathcal{D}^{(1)}} \right) ) \right), \\
...
\end{equation*}
so that two properties of the target are preserved in expectation: the average node degree matches \( \tilde{c} \), and the fraction of within-class (homophilous) edges matches \( \tilde{h} \). To see this, consider a node in a block of the average size \( \bar{b} \); treating the blocks as approximately equal in size, it has \( \bar{b} - 1 \approx \bar{b} \) potential neighbours inside its own community and \( \tilde{n} - \bar{b} \) potential neighbours outside it, each realized independently with probability \( p_\text{in} \) and \( p_\text{out} \) respectively. Its expected degree is therefore
\begin{equation*}
	\bar{b} \, p_\text{in} + \left( \tilde{n} - \bar{b} \right) p_\text{out} = \tilde{h} \, \tilde{c} + \left( 1 - \tilde{h} \right) \tilde{c} = \tilde{c},
\end{equation*}
recovering the target average degree. Of these edges, the within-community ones contribute \( \bar{b} \, p_\text{in} = \tilde{h} \, \tilde{c} \) in expectation, a fraction \( \tilde{h} \, \tilde{c} / \tilde{c} = \tilde{h} \) of the total, so the expected edge homophily equals \( \tilde{h} \) as well. The edges are then sampled from the SBM as an undirected graph, and each node is assigned the label of its block.
...
For the descriptor \( \delta \left( \mathcal{D} \right) \) we reuse the feature set of \cite{dedic_benchmarking_2026}, whose entries can be grouped by whether they concern the graph \textbf{structure} (topology), the node \textbf{attributes}, the \textbf{task} (labels, for multi-class problems averaged over all one-vs-other cases), or the interactions among these:
